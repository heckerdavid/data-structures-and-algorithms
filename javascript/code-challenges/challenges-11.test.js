...
const divisibleByFiveTwoToThePower = (input) => {
  let filteredArray = input.map(array => array.filter(number => number % 5 === 0));
  let doubleFilter = filteredArray.map(array => array.filter(number => typeof(number) === 'number'));

  let newArray = [];
  for (let i = 0; i < doubleFilter.length; i++){
    newArray.push([]);
    for (let j = 0; j <= doubleFilter.length; j++){
      if (doubleFilter[i][j]) {
        newArray[i].push(Math.pow(2, doubleFilter[i][j]));
      }
    }
  }
  return newArray;

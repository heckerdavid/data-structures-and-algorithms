...
const raisedToTheThird = (arr) => {
  const thirdPoweredArray = [];

  arr.forEach( number => thirdPoweredArray.push(number * number * number)
  );

  return thirdPoweredArray;
};
...
const addOne = (arr) => {
  let incrementedArray = arr.map( number => number + 1);
...
const addQuestion = (arr) => {
  let makeQuestionArray = arr.map( str => str + '?');

  return makeQuestionArray;
};
...
const extractAbilities = arr => arr.map(chara => chara.ability.name);
